...
from cpfl.core.datasets.Partitioner import DirichletDataPartitioner, DataPartitioner, KShardDataPartitioner
from cpfl.core.mappings.Mapping import Mapping
from cpfl.util.utils import Cutout
...
class STL10(Dataset):
    ...
    def load_trainset(self):
        ...
        if STL10.STL10_trainset:
            trainset = STL10.STL10_trainset
        else:
            trainset = torchvision.datasets.STL10(
                root=self.train_dir, split="train+unlabeled", download=True, transform=self.train_transformer
            )
            STL10.STL10_trainset = trainset
        ...
        if self.partitioner == "iid":
            ...
            raise RuntimeError("Unknown partitioner %s for STL10 dataset", self.partitioner)
    ...
    def test(self, model, device_name: str = "cpu"):
        ...
        self.logger.debug("Device for STL10 accuracy check: %s", device)
